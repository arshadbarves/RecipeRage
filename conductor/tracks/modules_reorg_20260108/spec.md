# Specification: Modules Reorganization and Scalability Refactor

## Overview
This track focuses on reorganizing the `Assets/Scripts` directory to extract common, reusable systems into a high-level "Modules" structure. The goal is to create broad, scalable categories (Core, Networking, UI) that can be reused across projects. A central component is the refactoring of `CurrencyService` into a comprehensive `BankService` (located within the Core module) that handles all player persistence data, including currency, inventory (skins), and progression, using an abstracted backend (EOS Player Storage).

## Functional Requirements
- **Directory Restructuring**:
    - Create `Assets/Scripts/Modules/` root.
    - Define broad module categories: `Core`, `Networking`, `UI`.
- **Core Module (`Assets/Scripts/Modules/Core/`)**:
...
- **Networking Module (`Assets/Scripts/Modules/Networking/`)**:
    - Move generic connection logic and networking foundations here.
- **UI Module (`Assets/Scripts/Modules/UI/`)**:
...
## Acceptance Criteria
- [ ] `Assets/Scripts/Modules/` contains only high-level folders: `Core`, `Networking`, `UI`.
- [ ] `BankService` is implemented in `Modules/Core/` and successfully handles:
    - [ ] Currency balances.
    - [ ] Inventory items (Skins).
    - [ ] Progression data.
- [ ] An EOS implementation for `BankService` persistence is functioning.
- [ ] Core utilities (Audio, Input, etc.) are successfully moved to `Modules/Core/`.
