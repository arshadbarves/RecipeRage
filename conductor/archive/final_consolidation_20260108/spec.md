...
## Overview
This final track completes the reorganization of the codebase into a strict two-bucket system: **Core** (Reusable, Agnostic) and **Gameplay** (Game-specific). It removes the remaining legacy top-level folders (`App`, `UI`) and streamlines `Core`.
...
## Namespace Policy
- All generic tools must be under `Core.*`.
