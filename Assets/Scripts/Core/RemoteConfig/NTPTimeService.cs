...
namespace Core.RemoteConfig
{
    ...
    public class NTPTimeService : INTPTimeService
    {
        ...
        private const int NTP_TIMEOUT_MS = 5000;
        ...
        public async UniTask<bool> SyncTime()
        {
            for (int attempt = 0; attempt < MAX_RETRY_ATTEMPTS; attempt++)
            {
                try
                {
                    GameLogger.Log($"NTP sync attempt {attempt + 1}/{MAX_RETRY_ATTEMPTS}...");
                    
                    var ntpTime = await FetchNTPTime();
                    ...
                    GameLogger.Log($"NTP sync successful. Offset: {_timeOffset.TotalSeconds:F2}s");
                ...
                {
                    GameLogger.LogWarning($"NTP sync attempt {attempt + 1} failed: {ex.Message}");
                    
                    if (attempt < MAX_RETRY_ATTEMPTS - 1)
                    {
                        // Exponential backoff
                        int delayMs = BASE_RETRY_DELAY_MS * (int)Math.Pow(2, attempt);
                        await UniTask.Delay(delayMs);
                    }
                }
            }
            
            GameLogger.LogError($"NTP sync failed after {MAX_RETRY_ATTEMPTS} attempts");
            return false;
        }
        ...
        /// </summary>
        private async UniTask<DateTime> FetchNTPTime()
        {
            // NTP message format (48 bytes)
            var ntpData = new byte[48];
            ntpData[0] = 0x1B; // LI = 0 (no warning), VN = 3 (IPv4), Mode = 3 (Client)
            
            IPAddress[] addresses = await Dns.GetHostAddressesAsync(NTP_SERVER);
            if (addresses.Length == 0)
            {
                throw new Exception($"Could not resolve NTP server: {NTP_SERVER}");
            }
