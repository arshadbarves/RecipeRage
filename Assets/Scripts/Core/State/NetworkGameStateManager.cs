...
namespace Core.State
{
    ...
    public class NetworkGameStateManager : NetworkBehaviour
    {
        ...
        public void RequestStartGameServerRpc(ServerRpcParams rpcParams = default)
        {
            ...
            {
                Debug.LogWarning($"[NetworkGameStateManager] Only host can start the game");
                return;
            }
        ...
        public void ChangePhase(GamePhase newPhase, float duration)
        {
            if (!IsServer)
            {
                Debug.LogWarning("[NetworkGameStateManager] Only the server can change game phase");
                return;
            }
            ...
            Debug.Log($"[NetworkGameStateManager] Changed phase to {newPhase} for {duration} seconds");
        }
        ...
        private void OnCurrentPhaseChanged(GamePhase previousValue, GamePhase newValue)
        {
            ...
            Debug.Log($"[NetworkGameStateManager] Phase changed from {previousValue} to {newValue}");
        }
        ...
        private void ChangePhaseClientRpc(GamePhase newPhase, float duration)
        {
            ...
            Debug.Log($"[NetworkGameStateManager] Client received phase change to {newPhase}");
        }
        ...
        public void ShowCountdownClientRpc(int seconds)
        {
            ...
            Debug.Log($"[NetworkGameStateManager] Countdown: {seconds}");
        }
    }
