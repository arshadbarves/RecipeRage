...
namespace UI.Components.Tabs
{
    ...
    public class SettingsTabComponent : ITabComponent
    {
        ...
        public void Initialize(VisualElement root, ServiceContainer services)
        {
            ...
            var animator = _services?.AnimationService?.UI;
        ...
        private void AddSettingsTab(string id, string btnName, string templatePath, Func<VisualElement, ITabComponent> factory)
        {
            ...
            if (btn != null && template != null)
            {
                ...
                component.Initialize(contentRoot, _services);
        ...
        private void OnApplyClicked()
        {
            ...
            var settingsService = _services?.SettingsService;
            settingsService?.ApplyAllSettings(_saveService.GetSettings());

            _services?.UIService?.ShowNotification("Settings applied", NotificationType.Success, 2f);
        }
        ...
        private void OnResetClicked()
        {
            ...
            _services?.UIService?.ShowNotification("Settings reset", NotificationType.Info, 2f);
        }
        ...
        #region Internal Sub-Tab Implementations
                ...
                public abstract class BaseSettingsTab : ITabComponent
                {
                    ...
                    public virtual void Initialize(VisualElement root, ServiceContainer services) 
                    {
                        _root = root;
                        Services = services;
                    }
                ...
                private class GraphicsSettingsTab : BaseSettingsTab
                {
                    ...
                    public override void Initialize(VisualElement root, ServiceContainer services)
                    {
                        base.Initialize(root, services);
                ...
                private class ControlsSettingsTab : BaseSettingsTab
                {
                    ...
                    private void OnEditControlsClicked()
                    {
                        ...
                        Services?.UIService?.ShowScreen(UIScreenType.HUDEditor);
                    }
                }
                ...
                private class AccountSettingsTab : BaseSettingsTab
                {
                    ...
                    private void OnCopyIdClicked()
                    {
                        ...
                        Services?.UIService?.ShowNotification("Player ID copied", NotificationType.Info, 2f);
                    }
        
                    private async void OnLogoutClicked()
                    {
                        GameLogger.Log("Requesting Logout");
                        var auth = Services?.AuthenticationService;
                        if (auth != null)
                        {
