...
namespace Gameplay.Stations
{
    ...
    public class StationNetworkController : NetworkBehaviour
    {
        ...
        public void RequestUseStationServerRpc(ulong playerId, ServerRpcParams rpcParams = default)
        {
            ...
            {
                Debug.LogWarning($"[StationNetworkController] Player {playerId} tried to use station but sender was {rpcParams.Receive.SenderClientId}");
                return;
            }
            ...
            if (_isLocked.Value && _currentUserId.Value != playerId)
            {
                ...
                {
                    Debug.Log($"[StationNetworkController] Station is locked by player {_currentUserId.Value}");
                    return;
                }
            }
            ...
            Debug.Log($"[StationNetworkController] Player {playerId} started using station");
        }
        ...
        public void ReleaseStationServerRpc(ulong playerId, ServerRpcParams rpcParams = default)
        {
            ...
            if (rpcParams.Receive.SenderClientId != playerId)
            {
                Debug.LogWarning($"[StationNetworkController] Player {playerId} tried to release station but sender was {rpcParams.Receive.SenderClientId}");
                return;
            }
            ...
            {
                Debug.LogWarning($"[StationNetworkController] Player {playerId} tried to release station but it's being used by {_currentUserId.Value}");
                return;
            }
            ...
            Debug.Log($"[StationNetworkController] Player {playerId} released station");
        }
        ...
        public void SetState(StationState newState)
        {
            if (!IsServer)
            {
                Debug.LogWarning("[StationNetworkController] Only the server can set station state");
                return;
            }
