...
namespace KitchenClash.Infrastructure.Persistence
{
    ...
    public class EconomyData : ISerializationCallbackReceiver
    {
        ...
        {
            _balanceKeys.Clear(); _balanceValues.Clear();
            foreach (var kvp in Balances) { _balanceKeys.Add(kvp.Key); _balanceValues.Add(kvp.Value); }
            _inventoryItems.Clear();
            foreach (var item in Inventory) _inventoryItems.Add(item);
            _dataKeys.Clear(); _dataValues.Clear();
            foreach (var kvp in Data) { _dataKeys.Add(kvp.Key); _dataValues.Add(kvp.Value); }
        }
        ...
        public void OnAfterDeserialize()
        {
            ...
            foreach (var item in _inventoryItems) Inventory.Add(item);
